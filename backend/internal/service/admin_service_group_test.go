...
func TestAdminService_CreateGroup_NormalizesCodexWSOnlyByPlatform(t *testing.T) {
	for _, tt := range []struct {
		name     string
		platform string
		want     bool
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			group, err := svc.CreateGroup(context.Background(), &CreateGroupInput{
				Name: "ws-only-" + tt.name, Platform: tt.platform, RateMultiplier: 1, CodexWSOnly: true,
			})

			require.NoError(t, err)
			require.NotNil(t, group)
			require.Equal(t, tt.want, repo.created.CodexWSOnly)
		})
	}
}

func TestAdminService_UpdateGroup_CodexWSOnlyOmittedPreservesAndFalseDisables(t *testing.T) {
	existingGroup := &Group{
		ID: 1, Name: "existing-ws-only", Platform: PlatformOpenAI, Status: StatusActive, CodexWSOnly: true,
	}
	repo := &groupRepoStubForAdmin{getByID: existingGroup}
	svc := &adminServiceImpl{groupRepo: repo}
	...
	require.NoError(t, err)
	require.True(t, repo.updated.CodexWSOnly)

	disabled := false
	repo.getByID = repo.updated
	_, err = svc.UpdateGroup(context.Background(), existingGroup.ID, &UpdateGroupInput{CodexWSOnly: &disabled})
	require.NoError(t, err)
	require.False(t, repo.updated.CodexWSOnly)
}

func TestAdminService_UpdateGroup_ClearsCodexWSOnlyWhenPlatformChanges(t *testing.T) {
	existingGroup := &Group{
		ID: 1, Name: "existing-ws-only", Platform: PlatformOpenAI, Status: StatusActive, CodexWSOnly: true,
	}
	repo := &groupRepoStubForAdmin{getByID: existingGroup}
	...
	_, err := svc.UpdateGroup(context.Background(), existingGroup.ID, &UpdateGroupInput{Platform: PlatformAnthropic})
	require.NoError(t, err)
	require.False(t, repo.updated.CodexWSOnly)
}

func TestAdminService_UpdateGroup_CodexWSOnlyInvalidatesAuthCache(t *testing.T) {
	existingGroup := &Group{ID: 1, Name: "existing-ws-only", Platform: PlatformOpenAI, Status: StatusActive}
	repo := &groupRepoStubForAdmin{getByID: existingGroup}
	invalidator := &authCacheInvalidatorStub{}
	svc := &adminServiceImpl{groupRepo: repo, authCacheInvalidator: invalidator}
	enabled := true

	_, err := svc.UpdateGroup(context.Background(), existingGroup.ID, &UpdateGroupInput{CodexWSOnly: &enabled})
	require.NoError(t, err)
	require.True(t, repo.updated.CodexWSOnly)
