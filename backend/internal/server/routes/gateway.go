...
func RegisterGatewayRoutes(
	...
	// 分组级「仅允许 Codex 官方客户端」：与白名单同链、排在其前，客户端身份不符先于模型校验拒绝。
	groupCodexCLIOnly := middleware.GroupCodexCLIOnly(settingService, cfg)
	...
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupCodexCLIOnly, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupCodexCLIOnly, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
	...
	antigravityV1.Use(groupCodexCLIOnly)
	...
	antigravityV1Beta.Use(groupCodexCLIOnly)
