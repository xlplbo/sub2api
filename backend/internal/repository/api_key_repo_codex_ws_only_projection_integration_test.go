...
func TestGetByKeyForAuthCarriesGroupCodexWSOnly(t *testing.T) {
	ctx := context.Background()
	suffix := time.Now().UnixNano()
	group := mustCreateGroup(t, integrationEntClient, &service.Group{
		Name: fmt.Sprintf("ws-only-proj-group-%d", suffix), Platform: service.PlatformOpenAI,
		RateMultiplier: 1, CodexWSOnly: true,
	})
	user := mustCreateUser(t, integrationEntClient, &service.User{
		Email: fmt.Sprintf("ws-only-proj-%d@example.com", suffix), Concurrency: 5,
	})
	groupID := group.ID
	keyValue := fmt.Sprintf("sk-ws-only-proj-%d", suffix)
	apiKeyRepo := NewAPIKeyRepository(integrationEntClient, integrationDB)
	key := &service.APIKey{UserID: user.ID, GroupID: &groupID, Key: keyValue, Name: "ws-only-proj", Status: service.StatusActive}
	...
	require.True(t, got.Group.CodexWSOnly)
}
